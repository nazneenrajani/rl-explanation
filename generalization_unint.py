...
class NoiseWrapper(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        ...
        # Generate noise and turn on the noisy TV in the agent's observation
        noise = np.random.normal(loc=self.mean, scale=self.std, size=observation.shape[:2])
        observation[:, :, 2] = noise
        return observation
...
def run():
    ...
    def wrap(env, noise, noise_setting):
    ...
    def make_env(setting, noise, noise_setting, rank, seed=0):
        def _init():
            env = wrap(GridExample(*setting), noise, noise_setting)
            env.seed(seed + rank)
            return env
    ...
    env_settings = [((0, 5, None, None), (0., 0.5)),
                    ((0, 2, None, None), (0., 0.5)),
                    ((0, 5, 1, None), (0., 0.5)),
                    ((0, 5, 2, 3), (0., 0.5)),
    ...
    try:
        evaluation_matrix, distances = pickle.load(open('fat/exp2/eval_data', 'rb'))
    except FileNotFoundError:

        # Go over each of the environments
        for i, (setting, noise_setting) in enumerate(env_settings):
            unint_models.append([])
            for j in range(5):
                # Store renderings of the source environment
                env = wrap(GridExample(*setting), noise_level, noise_setting)
                env.reset()
                ...
                if algorithm != SAC:
                    env = SubprocVecEnv([make_env(setting, noise_level, noise_setting, i) for i in range(num_cpu)])
                else:
                    env = DummyVecEnv([make_env(setting, noise_level, noise_setting, 0)])
                model = algorithm(CustomPolicy, env, verbose=1)
                try:
                    # Loading the model
                    model = algorithm.load(f"fat/exp2/{'.'.join([str(e) for e in setting]) + '.' + str(noise_level) + '.' + str(j)}.{postfix}")
                except ValueError:
                    # Training the model
                    model.learn(total_timesteps=500000)
                    model.save(f"fat/exp2/{'.'.join([str(e) for e in setting]) + '.' + str(noise_level) + '.' + str(j)}.{postfix}")

                unint_models[-1].append(model)
        ...
        for i, setting in enumerate(env_settings):
            ...
            make_gif(models[i][0], env, f"fat/exp2/gif_{'.'.join([str(e) for e in setting])}.gif")
    ...
    pickle.dump((evaluation_matrix, distances), open('fat/exp2/eval_data', 'wb'))
    ...
    plot_matrix(evaluation_matrix_mod,
                ...
                f'fat/exp2/evaluation_matrix_{postfix}_mod.png', 0, 1, mask)
    ...
    plot_matrix(evaluation_matrix,
                ...
                ['.'.join([str(e) for e in s]) for s in env_settings],
                'Target Task', 'Source Task',
                f'fat/exp2/evaluation_matrix_{postfix}.png', 0, 1)
    ...
    # plt.tight_layout()
    # plt.savefig(f'fat/exp2/evaluation_matrix_{postfix}.png')
    # plt.close()
    ...
    plot_matrix(success_matrix,
                ...
                ['.'.join([str(e) for e in s]) for s in env_settings],
                'Target Task', 'Source Task',
                f'fat/exp2/success_matrix_{postfix}.png', 0, 1)
    # plt.figure(figsize=(10, 8))
    ...
    # plt.tight_layout()
    # plt.savefig(f'fat/exp2/success_matrix_{postfix}.png')
    # plt.close()
    ...
    for i, dist_mat in enumerate(distances):
        ...
        plot_matrix(dist_mat_mod,
                    ...
                    labels,
                    'Target Task', 'Source Task',
                    f'fat/exp2/distance_matrix_{distance_types[i].lower()}_mod.png', mask=mask)
        ...
        plot_matrix(dist_mat,
                    ...
                    ['.'.join([str(e) for e in s]) for s in env_settings],
                    'Target Task', 'Source Task',
                    f'fat/exp2/distance_matrix_{distance_types[i].lower()}.png')
        ...
        # plt.tight_layout()
        # plt.savefig(f'fat/exp2/distance_matrix_{distance_types[i].lower()}.png')
        # plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'fat/exp2/dist_correlation_matrix.png')
    plt.close()
...
#
# if __name__ == '__main__':
#     env = GridExample(height=3, width=5)
#     env = NoiseWrapper(MiniGridNoisyTransitions(MiniGridForwardBackwardUpDownActionsOnly(FullyObsWrapper(env)), 0.),0,1)
#
#     obs = env.reset()
#     print (obs)
